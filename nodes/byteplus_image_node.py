...
class Seedream4UnifiedNode:
    """Seedream 4 Generation Node - Handles text-to-image, image-to-image, multi-image blending with batch generation"""
    ...
    CATEGORY = "Seed/ImageGeneration"
    DESCRIPTION = """
Seedream 4 unified generation node supporting text-to-image, image-to-image,
    ...
    def generate_image(self, prompt, width, height, batch_mode, batch_size, seed=-1, **kwargs):
        try:
            ...
            if batch_mode == "parallel":
                print(f"Seedream 4: Parallel batch mode, submitting {batch_size} concurrent API requests")

                def generate_single_image(index):
                    """Generate a single image in a thread"""
                    print(f"Seedream 4: Submitting request {index+1}/{batch_size}")
                    arguments = {
                        "prompt": prompt,
                        "size": size,
                        "response_format": "url",
                    ...
                    print(f"Seedream 4: Request {index+1} using random seed {request_seed}")
                    ...
                    print(f"Seedream 4: Request {index+1}/{batch_size} completed")
                    return images
                ...
                with ThreadPoolExecutor(max_workers=batch_size) as executor:
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            print(f"Seedream 4: Error in parallel request: {str(e)}")

                if all_images:
                    # Stack all images into a batch
                    stacked_images = np.stack(all_images, axis=0)
                    img_tensor = torch.from_numpy(stacked_images)
                    print(f"Seedream 4: Parallel batch complete, generated {len(all_images)} images")
                    return (img_tensor,)
                else:
                    return SeedResultProcessor.create_blank_image()
            ...
            # Handle single or sequential mode - single API call
                ...
                if batch_mode == "sequential":
                    ...
                    print(f"Seedream 4: Sequential batch mode, requesting max {batch_size} images (with {len(images_base64)} input images)")
                else:
                    # Single image generation
                    arguments["sequential_image_generation"] = "disabled"
                    print(f"Seedream 4: Single image mode")

                # Add images if provided
                if len(images_base64) == 1:
                    arguments["image"] = images_base64[0]
                    print(f"Seedream 4: Using image-to-image mode")
                elif len(images_base64) > 1:
                    arguments["image"] = images_base64
                    print(f"Seedream 4: Using multi-image blending mode ({len(images_base64)} images)")
                else:
                    print(f"Seedream 4: Using text-to-image mode")
                ...
                print(f"Seedream 4: API returned {len(result.get('data', []))} images")
                if 'usage' in result:
                    print(f"Seedream 4: Usage - generated_images: {result['usage'].get('generated_images', 'N/A')}")
